...
echo Using FPC: %FPC%
"%FPC%" -n @scripts\fpc-x64.cfg "..\vendors\uos\spike\uos_spike.pas" -FU"..\vendors\uos\spike\build\dcu" -FE"..\vendors\uos\spike\build" -ouos_spike.exe
if %ERRORLEVEL% neq 0 (
    echo UOS SPIKE BUILD FAILED
    popd
    exit /b %ERRORLEVEL%
)
...
popd
echo.
echo Build successful: vendors\uos\spike\build\uos_spike.exe
exit /b 0
