...
- **Окно/нарезка — из `config.json`:** `stream_window_ms` (дефолт 30000), `stream_chunk_ms`
  (дефолт 120000 = 2 мин). Длинное аудио режется на **последовательные chunk-сессии**: каждая сессия
...
- **Buffered-режим (`auto_flush=false`):** файловый драйвер ставит в `session_start` флаг
  `auto_flush=false` — демон тогда **не** пере-инференсит растущий буфер на каждый кадр, а копит chunk
  и делает **один** инференс на `flush` (O(n) вместо O(n²) на редкой речи). Живой стриминг
  (EchoRecorder) флага не шлёт → прежнее поведение с авто-flush по границам предложений.
